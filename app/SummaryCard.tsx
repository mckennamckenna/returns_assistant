...
export function SummaryCard({ count, dollarAmount, href }: { count: number; dollarAmount: string; href: string }) {
  return (
    <div className="mb-[22px]">
      ...
      <div className="bg-card border border-border rounded-2xl p-[18px] flex items-center gap-4">
        <span className="font-serif text-[34px] font-semibold text-ink leading-none">{count}</span>
        <span className="w-px self-stretch bg-border" />
        <span className="font-serif text-[25px] font-semibold text-ink leading-none">{dollarAmount}</span>
        <Link
          href={href}
