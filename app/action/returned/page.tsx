...
// The order-detail block shown above the confirm button — reads entirely
...
// button; the redemption decision itself lives in the endpoint.
function OrderSummary({ order, orderId }: { order: ReturnedOrderDetails; orderId: string }) {
...
export default async function ReturnedActionPage({
  searchParams,
}: {
  ...
  const verifyResult = verifyToken(token, { action: "returned" });
  ...
  const state = decideReturnedPageState(verifyResult, redemption, order);
  ...
  switch (state.state) {
    ...
    case "confirm": {
      // Invariant: decideReturnedPageState only returns "confirm" when
      ...
      return (
        <main className="min-h-screen flex items-center justify-center p-8">
          <div className="w-full max-w-sm text-center">
            <span className="text-xl font-semibold text-stone-800">Return Window</span>
            <h1 className="text-lg font-medium text-stone-800 mt-6">Mark {orderLabel(state.order)} as returned?</h1>
            <p className="text-stone-500 text-sm mt-2">
              This confirms you've shipped it back. Your refund check-in reminder will follow later.
            </p>
            <OrderSummary order={state.order} orderId={verifyResult.payload.orderId} />
            <form method="POST" action="/api/action/returned" className="mt-6">
