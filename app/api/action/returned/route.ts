...
// (app/action/returned/done/page.tsx) is purely a display concern reading
...
function redirectToOutcome(request: NextRequest, outcome: string): NextResponse {
  const url = new URL("/action/returned/done", request.url);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const outcome = await prisma.$transaction(async (tx) => {
      ...
      const decision = decideReturnedOutcome(order, { userId: payload.userId });

      if (decision.shouldMarkReturned && order) {
        const data = buildStatusTransitionData("returned", order);
