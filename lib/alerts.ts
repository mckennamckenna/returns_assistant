...
export async function getAlertOrders(userId: string, now: Date = new Date()): Promise<OrderCardOrder[]> {
  const openOrders = await prisma.order.findMany({
    ...
    // OrderCard's field set (OrderCardOrder) plus needsReview, which this
    // function's own filter below needs but OrderCard itself doesn't render.
    // This previously fetched full Order rows on every page navigation
