...
// "ordered"/"shipped"/"return_requested" only — deliberately excludes
// "returned": that means the user already acted (shipped it back), so
// there's no missed window to sweep, and it's already tracked separately by
// the refund check-in cron (lib/refundCheckin.ts). "refunded" and "kept" are
// already auto-archived on their own transitions, so they'd never match
// activeOrderFilter here anyway.
//
...
const NON_TERMINAL_STATUSES = ["ordered", "shipped", "return_requested"];
