...
|---|---|---|---|---|---|---|---|---|
| `POST /api/inbound` | ✓ Basic Auth (C1, resolved) | ✓ (token→user) | ✓ (H1, 30/hr per token) | ⚠︎ C2 (LOW, accepted) | ✓ | ⚠︎ M2 (primary) / L4 (accepted risk) | — | ✓ |
...
### C1 — Inbound webhook has no sender verification, and attributes mail by a non-secret token — ✅ RESOLVED 2026-07-19 (3 of 4 parts done; part 4 consciously rejected, not outstanding)
...
3. **Rotate `inboundToken` to high entropy — ❌ rejected 2026-07-19, not pursued.** Re-assessed below (d); the quantified entropy math doesn't support this as the real gap.
...
**Resolved 2026-07-19.** The real residual gap was **no sender authentication on inbound mail** — tracked as **C2** below, now accepted (downgraded to LOW; see C2's entry for why). With C2 accepted as its own tracked finding, and the 4th part (entropy rotation) consciously rejected as low-value rather than left outstanding, C1 carries no remaining open/unresolved marker.

---

### C2 — ACCEPTED, downgraded CRITICAL → LOW 2026-07-19 — Inbound processing trusts sender identity unconditionally — no SPF/DKIM/authentication check
...
**Attacker scenario.** An attacker who knows or obtains a user's forwarding address (see C1(a) — a low bar, since the address is semi-public by design) sends a forged-sender email — e.g. spoofing `returns@nordstrom.com` — directly to that address. If Postmark's own inbound processing doesn't independently reject it (platform config, unconfirmed — see C1(f)), it lands exactly as a genuine forward would: extraction runs and a fabricated order or fake refund confirmation can be created on the victim's own dashboard. This reproduces the full impact of the original C1 attacker scenario without needing to forge the webhook request at all — email was always a sufficient delivery mechanism, C1's fix only closed the non-email delivery path.

**Why downgraded from CRITICAL to LOW (owner decision, 2026-07-19).** The original CRITICAL rating double-counted two findings that don't actually depend on C2: **M2** (the return-portal phishing risk) requires no forged sender at all — a genuinely-forwarded email can carry attacker-influenced body text just as easily via prompt injection, so M2 is a real risk with or without C2 — and **L4** (auto-advance via a forged refund email) is now its own accepted risk under the current trusted-alpha threat model (see L4's entry), so it no longer contributes to C2's urgency either. Once isolated from those two, C2's own genuine residual is narrower: a forged-sender email can create a fabricated order or corrupt a legitimate one's *own* dashboard state (single-account integrity), and adds to the rate-limited LLM cost-exhaustion concern H1 already bounds the *rate* of. No cross-user data leak and no privilege escalation — the blast radius stays inside the one account the forged mail was sent to. That residual is real but LOW, not CRITICAL.

**Remediation direction (narrowed, 2026-07-19).** Conservative flag only: forwarded mail whose original-sender authenticity can't be verified from a trusted hop gets `needsReview` — same "don't silently trust it, but don't block it either" posture as the rest of this app's confidence handling. Explicitly **not** in scope: multi-provider sender-authentication parsing (SPF/DKIM/`Authentication-Results` handling across whatever mix of forwarding sources exist) — that's brittle to build and maintain, and the forwarding ingestion path itself is being demoted in priority by the planned Gmail-OAuth ingestion pivot, which sidesteps this class of problem entirely for users who move to it. Confirming what Postmark's inbound payload actually exposes for this account is still useful context for the flag (platform inspection, not a code question) but is no longer gating a CRITICAL-severity fix.
...
### M2 — PRIMARY OPEN FINDING (2026-07-19) — AI-extracted "return portal" URL is surfaced as a trusted link (phishing via prompt injection)
...
**Why this is now the primary open finding (2026-07-19).** With C1 resolved, C2 accepted at LOW, and L4 accepted as risk under the current trusted-alpha threat model, M2 is the highest-severity finding still genuinely open. It's independent of C1/C2 and not addressable by sender authentication: the malicious URL doesn't require a forged sender or any inbound-auth bypass — a genuinely-forwarded email can carry attacker-influenced body text just as easily via prompt injection. Its fix is also provider-agnostic: nothing about the remediation below depends on the Postmark-forwarding ingestion path, so it survives the planned Gmail-OAuth ingestion pivot unchanged — it's about what the app does with an already-extracted URL, not how the email arrived.
...
- **L4 — ACCEPTED RISK 2026-07-19 (not open, not fixed) — Prompt-injection-driven status changes.** Extracted `refundAmount`/`emailType` can auto-advance an order's `displayStatus` (e.g. a forged "refund" email → order shows refunded). Impact is confined to the victim's own view and reminder suppression, so it's low, but it's the clearest example of extracted content driving control flow. Direction: keep money/status-affecting transitions behind the existing `needsReview` gate for low-confidence or injection-suspect extractions.
  ...
  **Accepted 2026-07-19 — current trusted-alpha threat model.** Owner has accepted that no actor is maliciously auto-advancing another user's order status at this stage of the app — every user today is a trusted alpha participant, not an adversarial population. **Revisit trigger: re-open the moment the app admits non-trusted users** (public signup, or any user population beyond the current trusted cohort).
