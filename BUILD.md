...
- Auto-advance rules (in `deriveDisplayStatus`): `return_label` → `return_requested`; `deliveredAt != null` → `delivered`; else `shipping_confirmation` or `delivery` (with no `deliveredAt`) → `shipped`; else → `ordered`. No auto-advance past `return_requested` — **except** a `refund`-type email (see below), the one auto-derivation signal allowed to move an order past `return_requested`/`returned` on its own.
- **`delivered` (AquaTru "Shipped forever" fix, 2026-07-21):** derived off the persisted `Order.deliveredAt` field, deliberately **not** off the triggering email's type — the state must stay stable across later emails, not only true in the moment a `delivery` email is processed. Sits just above `shipped`, below every return-related rung, so an order that's both delivered and mid-return shows the more-advanced return state (a `return_label` arriving on a `delivered` order still advances to `return_requested`). `delivered` and the internal `status === "returnable"` field are deliberately orthogonal — this badge answers "where's my package," `status`/the return-deadline chip answers "how long to return"; an order with no delivery confirmation correctly stays `"shipped"`, never fabricated as delivered. Never a manual transition (`ALLOWED_MANUAL_STATUSES` deliberately excludes it) — only ever auto-derived. Renumbering every rank above it (return_requested/returned/kept/refunded all shifted up by one) was safe because every comparison site reads `DISPLAY_STATUS_RANK.<name>` by key, never a hardcoded numeric literal — verified by grep across the codebase before building. Required auditing every consumer of the displayStatus value set, not just rank comparisons: `lib/autoArchive.ts`'s `NON_TERMINAL_STATUSES` was a real gap (a name-based allowlist, not a rank threshold) — it needed `"delivered"` added or a delivered-but-unactioned order past its window would never get swept; `lib/reminders.ts`'s `SKIP_DISPLAY_STATUSES` and `weekly-digest/route.ts`'s `EXCLUDED_STATUSES` were checked and deliberately left unchanged (a delivered order's return clock is still ticking, so reminders/digest inclusion must continue). **Backfill required, and real:** `recomputeDisplayStatus()` only fires on new email ingestion, so any order with `deliveredAt` already set before this deploy would stay stuck at its old `displayStatus` indefinitely, not just until the next email — `scripts/backfill-delivered-status.ts` (dry-run/`--apply`, reuses `recomputeDisplayStatus()` directly) found and advanced 4 real orders to `"delivered"` on deploy day. **AquaTru itself — the motivating example — does NOT advance, checked directly against production:** its `deliveredAt` is `null` because neither of its linked `delivery`-typed emails states a date anywhere extractable — not in the AI extraction, and not in a forwarded-header `"Date:"` line (`parseForwardedHeaderDate()` returns null on both; the only date on either email is `receivedAt`, the forward/envelope date, not a confirmed delivery date). Using it as a stand-in would be exactly the fabrication this design deliberately avoids. A real, separate, project-wide gap (15/33 `delivery` emails have no captured date) — logged in `TASKS.md` 🟡 Next, not fixed here.
