...
- **Merge rule:** new non-null value wins; null never erases existing data. Exceptions: `order_confirmation` `orderTotal` is authoritative once present; `orderDate` is write-once (see below), not last-write-wins; and a `stated_in_email` return window is never replaced by a lower-provenance one (see below).
- **Fallback `orderDate`, gated by earliest-linked email's `emailType`:** if an Order has no `orderDate` after linking, `applyFallbackOrderDate` derives one from the earliest linked email — but only when that email's type is `order_confirmation`, `shipping_confirmation`, or `delivery` (not scoped to `order_confirmation` alone — Amazon's transactional mail never produces that emailType at all, only `shipping_confirmation`; Bug 8). `return_label`, `refund`, and `other`-typed earliest emails leave `orderDate` null and `orderDateEstimated` false — their `receivedAt` has no defined relationship to the true order date (post-purchase-loop mail, or for `other`, unrelated marketing), and inventing an anchor from them produced a visibly-wrong deadline in production (Caroline's Moda order, 2026-07-08; see Decisions log). **Any new `emailType` value added to the enum must be explicitly classified into the allowed or excluded bucket at the point of addition — there's no default.** For allowed types, two tiers: (1) parse the `Date:` line from a forwarded-message header in the email's body, handling both Gmail and Apple Mail forwarded-header formats, when present; (2) otherwise fall back to the email's own `receivedAt` (Postmark's parsed `Date` header, `app/api/inbound/route.ts`) — always present, and a good proxy for auto-forwarded/directly-relayed transactional mail. Either tier sets `orderDateEstimated: true` on the Order (distinct from `deadlineIsEstimated`, which stays coupled: an inferred order date always makes any deadline computed from it estimated too).
  **`orderDate` is write-once in `mergeEmailIntoOrder` (2026-08-16), not last-write-wins.** Once `Order.orderDate` is non-null, no later-linked email — of ANY type, including another establishing one — may change it; `orderDateEstimated` only clears to `false` the first time an establishing email (`order_confirmation`/`shipping_confirmation`/`delivery`) genuinely establishes it, and is left untouched on every merge after that. The establishing-type gate (`ALLOWED_FALLBACK_EMAIL_TYPES`) still governs what may set `orderDate` — but only as a first-write gate, not an ongoing overwrite permission. **Superseded text, kept for the historical record:** the sentence this replaces described `mergeEmailIntoOrder` clearing `orderDateEstimated` back to `false` whenever ANY later email supplied a genuinely-extracted `orderDate` — that was last-write-wins, and it's what caused the bug this fixes: a real production order's `orderDate` (order_confirmation, 2026-07-23) was silently overwritten to a `refund` email's own extracted date (2026-08-12) when the refund was linked weeks later, because the old rule let any non-null `email.orderDate` win outright. **Also insufficient on its own, and explicitly not what got built:** a same-type allowlist that permits ANY establishing-typed email to overwrite (not just refund/return_label/other) would still reproduce the bug one step removed — a `delivery` email arriving after `order_confirmation` is itself establishing, so it would still silently replace a correct earlier date with a later, wrong one (confirmed against a real order in the same investigation). Write-once — first establishing email wins, frozen after — is the shape that closes both failure modes. See TASKS.md, 2026-08-16.

  **Policy provenance in `mergeEmailIntoOrder` (2026-09-22): a `stated_in_email` return window is never replaced by a lower-provenance one.** When `Order.policySource` is `"stated_in_email"` **and the order has a non-null `returnWindowDays`**, and an incoming email carries a non-null `returnWindowDays` from any other source (`web_lookup`, `amazon_default`), the order keeps its existing `returnWindowDays`, `returnWindowStartsFrom` and `policySource` — all three together, as one unit, never a mix of the two sources. Everything else about the merge is unchanged: a null incoming window still never clears a resolved one, `web_lookup` vs `web_lookup` still resolves by recency (this merge has no tie-break between two lookups and the guard does not add one), and `stated_in_email` vs `stated_in_email` still resolves by recency. Comparison happens after `mapPolicySource()`, because the `Email` row spells this `"email"` and the `Order` row spells it `"stated_in_email"` for the same fact. **Why it exists:** TASKS.md 2026-09-21 H&M incident — a retailer-stated 30-day window was overwritten by a 3-day web-lookup answer merged in from a UPS carrier notification that carried no policy text of its own, moving the displayed deadline 27 days early with `deadlineIsEstimated: false`. The guard is modeled on the `orderDate` provenance rule directly above it; the two are deliberately the same shape. **Not covered:** `manual_override` and `user_supplied` remain unprotected — an open ranking decision, see TASKS.md 🔴 Now (2026-09-22).
...
- **Return-portal-link trust (M2, 2026-07-19):** `classifyReturnPortalTrust()` (`lib/extract.ts`) classifies the incoming email's `returnPortalUrl` + `retailer` into a tier — `known-third-party-portal` (a small vetted allowlist: Loop Returns, Narvar, parcelLab, Reveni, Linc confirmed live in our own data; Happy Returns, ReBOUND seeded from public knowledge, unverified against our data), `retailer-own-domain` (exact match only, never a substring/contains check — see the function's own comment on the short-domain-label collision risk), `web-lookup-sourced` (measurement-only, NOT a security boundary — `policySource` is an imprecise proxy for which source produced *this* URL specifically), or `unknown-unverified`. Only `unknown-unverified` forces `Order.needsReview: true`, same gate as the two forced-review paths above — a SIGNAL, never a hard block; `returnPortalUrl` still renders/opens exactly as before regardless of tier. Matching is on the **registrable domain only** (never the raw hostname/subdomain chain) — the core security property, since a hostname can contain the retailer's name as a subdomain label while a different party controls the actual domain (real observed shape: a Hanna Andersson marketing link hosted on `bloomreach.co`, a third-party ESP — the same shape an attacker domain like `amazon.evil-cdn.com` would exploit). Unlike the other forced-review paths above, this one writes no `userNote` — `lib/orderReview.ts`'s `reviewReasonLabel()` re-derives the reason live by re-running the classifier against the order's own stored fields, since it's a pure function of data already on the row rather than a point-in-time fact like the kept-conflict above. Tier distribution is logged at the call site, count-only (no URL/retailer/order id — some observed `returnPortalUrl` values carry PII in their query string), to measure how often `unknown-unverified` actually fires; a high rate would indicate an extraction-quality or domain-matching-heuristic problem, not a UI need. See the Decisions log entry on why this is a deliberate, named exception to the Order/Email `needsReview` separation above.
...
- Values and ranks: `ordered=1`, `shipped=2`, `delivered=3`, `return_requested=4`, `returned=5`, `kept=5`, `refunded=6`.
...
- Auto-advance rules (in `deriveDisplayStatus`): `return_label` → `return_requested`; `deliveredAt != null` OR a `delivery`-type email is linked → `delivered`; else `shipping_confirmation` → `shipped`; else → `ordered`. No auto-advance past `return_requested` — **except** a `refund`-type email (see below), the one auto-derivation signal allowed to move an order past `return_requested`/`returned` on its own.
- **`delivered` (AquaTru "Shipped forever" fix, 2026-07-21, widened 2026-07-23):** derived off `Order.deliveredAt != null` OR a confirmed `delivery`-type email being linked to the order — either is order-level, accumulated-history evidence (built from every email ever linked, not the single triggering one), so the state stays stable across later emails either way. Sits just above `shipped`, below every return-related rung, so an order that's both delivered and mid-return shows the more-advanced return state (a `return_label` arriving on a `delivered` order still advances to `return_requested`, confirmed for both signal paths). `delivered` and the internal `status === "returnable"` field are deliberately orthogonal — this badge answers "where's my package," `status`/the return-deadline chip answers "how long to return"; an order with no delivery evidence at all correctly stays `"shipped"`, never fabricated as delivered. Never a manual transition (`ALLOWED_MANUAL_STATUSES` deliberately excludes it) — only ever auto-derived. Renumbering every rank above it (return_requested/returned/kept/refunded all shifted up by one) was safe because every comparison site reads `DISPLAY_STATUS_RANK.<name>` by key, never a hardcoded numeric literal — reconfirmed by grep 2026-07-23, still clean. Required auditing every consumer of the displayStatus value set, not just rank comparisons: `lib/autoArchive.ts`'s `NON_TERMINAL_STATUSES` was a real gap (a name-based allowlist, not a rank threshold) — it needed `"delivered"` added or a delivered-but-unactioned order past its window would never get swept; `lib/reminders.ts`'s `SKIP_DISPLAY_STATUSES` and `weekly-digest/route.ts`'s `EXCLUDED_STATUSES` were checked and deliberately left unchanged (a delivered order's return clock is still ticking, so reminders/digest inclusion must continue).
  - **2026-07-21 build (deliveredAt-only) shipped with a real, self-diagnosed gap: AquaTru itself, the motivating example, did NOT advance.** Its `deliveredAt` is `null` because neither of its two `delivery`-typed emails states a date anywhere extractable — not in AI extraction, not in a forwarded-header `"Date:"` line. Requiring a *date*, not just a confirmed delivery-type *email*, was the actual bug.
  - **2026-07-23 fix: widened to treat a `delivery`-type email itself as sufficient evidence, independent of whether a date was extractable.** `deliveredAt` stays the more precise signal when present (a real confirmed date); the email-type signal is the fallback that closes the AquaTru gap. Verified directly against production after the backfill below: AquaTru now reads `displayStatus: "delivered"` with `deliveredAt` still honestly `null` — the badge and the date field are allowed to disagree, by design.
  - **Backfill, both rounds.** First backfill (2026-07-21, deliveredAt-only scope) found and advanced 4 real orders. The scope itself was the gap — it explicitly excluded any order without a persisted `deliveredAt`, which is exactly AquaTru's shape, so it could never have caught this class of order regardless of when it ran. Second backfill (2026-07-23, same script, widened candidate query: `deliveredAt != null OR a linked "delivery"-type email`) found 8 eligible orders (AquaTru among them, plus others whose `delivery` emails were manually-forwarded with no extractable date — same root shape, different retailers) and advanced all 8. `scripts/backfill-delivered-status.ts` reuses `recomputeDisplayStatus()` directly for `--apply` both times, so it can't drift from the live derivation path.
  - The remaining gap — `delivery` emails that state no date at all, project-wide (15/33 at last count, live webhook so this drifts) — no longer produces a wrong *badge*, only a still-honestly-null `deliveredAt`. Tracked in `TASKS.md` 🟡 Next as its own extraction-quality gap, not a displayStatus problem anymore.
...
- **Scoped to `displayStatus: { in: ["ordered", "shipped", "delivered", "return_requested"] }`**
  ...
  this sweep would consider them. `"delivered"` added 2026-07-21 alongside the
  displayStatus rung of the same name (see displayStatus above) — a
  delivered-but-unactioned order missing its window is exactly the silent-miss case
  this sweep exists to catch, same as `"shipped"`; leaving it out would have silently
  exempted every delivered order from ever being swept.
...
- **`Order.needsReview` and `Email.needsReview` serve two different jobs today**: linking-quality review (Order-level, user-facing "Looks correct / Split into separate order" UI) and extraction-quality review (Email-level, admin diagnostic). Extraction-quality signals are deliberately not propagated to `Order.needsReview` until a proper spec pass separates the two concerns. **Named exception (2026-07-19, M2/`classifyReturnPortalTrust`):** an unverifiable return-portal-link trust tier is, by this same definition, an extraction-quality signal — yet it *does* force `Order.needsReview` (see Order linking below). This is a deliberate owner decision, not a case that already fit the rule above: `returnPortalUrl` is rendered as a trusted, clickable action the user acts on directly (`SECURITY_AUDIT.md`'s M2), so its trust is treated as order-level product risk, not merely an admin-diagnostic extraction-quality concern the way a low-confidence field or a tiered-window note is. Do not read this as the general rule loosening — the next extraction-quality signal proposed for `Order.needsReview` still needs its own explicit justification, not a pointer to this one.
...
- **M2 (return-portal-link trust, 2026-07-19) feeds `Order.needsReview` directly, a deliberate named exception to "extraction-quality signals stay at `Email.needsReview` until a spec pass" (see that entry above) — not a case that already fit the rule.** `returnPortalUrl` is rendered as a trusted, clickable "Start a return" action; its trust is order-level product risk, not an admin-diagnostic concern. No new UI, badge, or trust-state visual — the pending review-disclosure spec is what handles presentation for all `needsReview` reasons uniformly, this is logic/plumbing only. `classifyReturnPortalTrust()`'s reason is deliberately re-derived live at render time (`reviewReasonLabel()`), not stored in a new field: it's a pure function of `returnPortalUrl`/`retailer`/`policySource`, all already on the row, unlike `computeKeptStatusConflict`'s reason, which depends on a point-in-time fact (was `displayStatus` `"kept"` *when* the conflicting email arrived) that isn't recoverable from current state alone. A shared, dedicated reason field for all `needsReview` triggers (superseding the existing `userNote`-parsing in `reviewReasonLabel()`, and the parked "move retailer-prefix marker off `userNote`" `TASKS.md` item) is the disclosure spec's data model to design once, informed by every reason type at once — not retrofitted from M2 alone.
...
- Deploy: **automatic on push.** `mckennamckenna/returns_assistant` is connected to this Vercel project via the GitHub integration (connected 2026-06-21) — every push to `main` triggers a production deploy on its own, typically live within a few seconds — **except** docs-only pushes: since 2026-09-19 `vercel.json`'s `ignoreCommand` skips the build when nothing but `*.md` changed since the last successful deploy (`VERCEL_GIT_PREVIOUS_SHA`; an empty or unreachable previous SHA builds). Deployment Retention: 7 days. Do **not** run `vercel --prod` — it creates a redundant duplicate deployment alongside the one GitHub already triggered. (Corrected 2026-07-19: this section previously said deploys were manual/no auto-deploy — verified wrong against actual Vercel deployment timestamps, which land ~6-7s after each commit, the signature of the GitHub webhook, not a manually-run CLI command.)
- Env vars: `npx vercel env add <NAME> <environment>` — new/changed vars take effect on the next deploy, not immediately.
- Verify: `npx vercel inspect returns-assistant.vercel.app | grep "Git Commit"` (or `app.myreturnwindow.com`) to confirm the live deploy matches the expected commit. Live may trail HEAD by docs-only commits; `git diff --quiet <live-sha> HEAD -- . ':(exclude)*.md'` exiting 0 means in sync.
