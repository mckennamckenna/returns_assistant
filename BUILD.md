...
## Subject line as an orderNumber source (fix, added post-Milestone 2)

**The problem:** Some retailers state the order number only in the email subject line and never repeat it in the body. Confirmed case: Proenza Schouler shipping confirmations arrive with subject "A shipment from order #86864 is on the way" — the body is a FedEx-style template that never mentions the order number again. Because `buildPrompt()` in `lib/extract.ts` received only the email body (the existing rule "extraction reads the body, not the headers" was sound reasoning for `retailer`, but over-applied to `orderNumber` as well), `orderNumber` came back null and the email was flagged `needsReview`. This caused a new, empty Order card to be created for that email instead of linking it to the existing Proenza Schouler order.

**The fix, in `lib/extract.ts`:** `buildPrompt()` now accepts a `subject` parameter alongside `textBody` and interpolates it as an `EMAIL SUBJECT:` section above `EMAIL BODY:` in the prompt. The extraction rules are updated with a new rule: "orderNumber may be read from the subject line as well as the body. retailer must NEVER be read from the subject or From header — body only." This preserves the original retailer constraint (a brand name appearing in a subject line like "Your SKIMS order has shipped!" is too ambiguous — it could be a marketing email) while allowing the less-ambiguous, numerically-scoped orderNumber to come from the subject.

`extractEmail()` signature changed from `extractEmail(textBody: string)` to `extractEmail(textBody: string, subject: string | null)`. `lib/runExtraction.ts` passes `email.subject ?? null`. The fallback in `buildPrompt` is `"(no subject)"` when subject is null.

**Backfill:** `scripts/backfill-subject-orderNumber.ts` re-runs extraction on every Email row where `orderNumber IS NULL AND needsReview = true`. Default is a dry run that prints matching row count + retailer/subject for each — run with `--apply` to execute. After re-extraction, `needsReview` clears automatically if orderNumber was the sole reason: the extraction function recomputes it from all conditions (confidence, deadline, unclear policy lookup) and a resolved orderNumber alone is enough to clear it. If any other condition still applies, the flag stays.

**How to know it works:** the Proenza Schouler shipping email (subject "A shipment from order #86864 is on the way") should now extract `orderNumber: "86864"` and `needsReview: false` after the backfill. The email should link to the existing Proenza Schouler order rather than creating a new card. Separately, a marketing email whose subject contains a brand name but whose body is promotional (no order content) should still yield `retailer: null`, confirming the subject-reads-retailer guard holds.

**Prompt 21 — subject-line orderNumber fix**
> Per BUILD.md's "Subject line as an orderNumber source" addendum to Milestone 2: update buildPrompt() in lib/extract.ts to accept a subject parameter and interpolate it as an EMAIL SUBJECT: section above EMAIL BODY:. Add a rule that orderNumber may be read from the subject as well as the body, but retailer must never come from the subject or From header. Update extractEmail()'s signature to accept subject: string | null and pass email.subject ?? null from lib/runExtraction.ts. Write a backfill script that re-extracts Email rows where orderNumber IS NULL AND needsReview = true, with a dry-run mode that prints matching rows before burning API calls. After backfill, only clear needsReview if orderNumber was the sole reason. Verify the Proenza Schouler shipping email (#86864) now resolves correctly.
...
- **Delivery-only orders during alpha.** 4 real orders (H&M, Freda Salvador, Tuckernuck, Shopbop) had only a delivery email linked — no `shipping_confirmation` — and `deriveDisplayStatus` had no mapping for `delivery`, so these stayed stuck at `"ordered"` despite the internal `status` being correctly `"returnable"`. Root cause: not a pipeline bug. During alpha, users forward emails manually and don't reliably forward every email a retailer sends, so the shipping confirmation simply never entered the system. Confirmed via investigation: no trace of a missing, discarded, or misclassified `shipping_confirmation` for any of the 4 orders. Fixed by adding `delivery → "shipped"` to `deriveDisplayStatus` (delivery is a strict superset of shipped — an order cannot be delivered without having shipped) plus a one-time `recomputeDisplayStatus` backfill across all 10 orders that were stuck at the migration default. Watch during beta: once real inbound automation (Gmail forwarding filter) replaces manual forwarding, delivery-only orders should become rare. If still common post-beta, re-investigate — that would point to real pipeline data loss (discard gate, extraction, missed webhook) rather than the user simply not forwarding.
...
# Milestone 20: Consolidate onto myreturnwindow.com

## Goal — the only thing that counts as "done"

> The dashboard loads from `app.myreturnwindow.com`, reminder emails arrive from `reminders@myreturnwindow.com`, and magic-link logins arrive from `hello@myreturnwindow.com` — all verified against real sends, not just by inspecting config.

No metaxmoda.com address sends on behalf of Return Window. No user-visible URL in the codebase references the old Vercel subdomain. The Postmark inbound webhook is a deliberate carve-out: both URLs serve the same deployed app, and rotating a working webhook carries real risk of dropped mail with no functional benefit.

## Why this design (read before building)

- **Subdomain over apex for the dashboard.** `app.myreturnwindow.com` is a CNAME at GoDaddy pointing at Vercel's CNAME target. A CNAME auto-follows any Vercel infrastructure changes without manual DNS updates. The apex (`myreturnwindow.com`) stays free for a future marketing or landing page — untangling an apex A/ALIAS record away from a deployed app is messier than it sounds, so keeping the root free now costs nothing.

- **`APP_URL` in `app/api/cron/route.ts` was the only hardcoded absolute URL in the codebase.** Everything else — Auth.js callback URLs, the Postmark inbound webhook registration, per-user address display — either derives its host from the incoming request at runtime or was already reading from an env var. One constant to change; both usages (the order-detail link in reminder email bodies and the admin-summary link) reference the constant, so the update propagated automatically.

- **Auth.js v5 `trustHost` means no env var was needed for login callbacks — and none was ever set.** Auth.js v5 auto-enables `trustHost` whenever the `VERCEL` environment variable is present (it always is on Vercel deployments), and when `AUTH_URL` is absent it derives the callback base URL from the incoming request's `Host` header. Browsing to `app.myreturnwindow.com` meant magic-link callbacks were already constructing themselves from that hostname with zero code or config change. This is worth calling out explicitly: a future migration should not go looking for a `NEXTAUTH_URL` or `AUTH_URL` that was deliberately never set, and should not add one unless something actually breaks. Adding either variable with a stale or incorrect value is a real way to break auth — as happened in Milestone 8 when outside advice introduced them mid-debugging.

- **Four DNS records at GoDaddy for Postmark outbound domain verification.** `myreturnwindow.com` had no prior email history, so SPF was a clean, standalone record — no merge with a pre-existing include, unlike `metaxmoda.com` where Google Workspace's `include:` was already in the root SPF and required care not to break it (Milestone 4). Records added: a DKIM TXT record per Postmark's Domains tab, a `pm-bounces` CNAME → `pm.mtasv.net` for bounce handling, a root SPF (`v=spf1 include:spf.mtasv.net ~all`), and a DMARC TXT at `p=none` to start — monitor-only, same initial posture as Milestone 4's DMARC for metaxmoda.com.

- **Login sender and reminder sender split into separate env vars.** `REMINDER_FROM_EMAIL` drives the daily cron reminders, the weekly alpha coverage check, and admin notifications. `LOGIN_FROM_EMAIL` drives magic-link login emails. `auth.ts` reads `LOGIN_FROM_EMAIL ?? REMINDER_FROM_EMAIL` as a fallback — if `LOGIN_FROM_EMAIL` is ever unset, behavior reverts to whatever `REMINDER_FROM_EMAIL` is, so the split is backward-compatible. The separation means the two addresses can be updated, suspended, or routed through different Postmark sender signatures independently without touching each other's flows.

- **The Postmark inbound webhook URL is deliberately not rotated.** The webhook is still registered against `returns-assistant.vercel.app/api/inbound`. That domain and `app.myreturnwindow.com` both serve the same Vercel deployment; forwarded emails still arrive correctly. Rotating the webhook would be a live-config change with real risk of dropping mail during the transition window for zero functional benefit. When the inbound domain itself is ever migrated (or the old Vercel URL is decommissioned), that's the moment to update it — not before.

- **Operational lesson: always name recipients before triggering a send.** During testing for this milestone, a `?force=true` cron run was triggered against production without first confirming who would receive mail. It sent real deadline reminders to three real users (two separate accounts, 13 orders total) and burned their `7_day` dedup rows — meaning the actual scheduled cron would have silently skipped those users' genuine 7-day reminders on the real threshold date. The dedup rows were cleaned up by deleting only those specific `Reminder` rows by id. The rule going forward: any action that sends email to real users — forced cron runs, test sends, one-off scripts, anything that calls `sendEmail` or Postmark directly — must explicitly name the recipients and wait for approval before executing.

- **Operational lesson: Vercel's "Sensitive" flag hides env-var values entirely, not just from the list preview.** During this milestone, `LOGIN_FROM_EMAIL` and `REMINDER_FROM_EMAIL` were both marked Sensitive in Vercel's dashboard. When both turned out to be silently blank (wrong or missing values), there was no way to read the current value back through the UI to diagnose the problem — the field shows only `*****` even in the edit view, unlike a non-Sensitive var which shows its value when you click to edit. This made two separate env-var failures harder to diagnose than they needed to be. Rule: mark Sensitive only for true secrets (API keys, auth tokens, cryptographic keys). Email addresses, feature flags, and public URLs should stay non-Sensitive so their values remain readable and verifiable when something breaks.

## What got built

1. **GoDaddy CNAME.** `app.myreturnwindow.com` → Vercel's CNAME target, added via GoDaddy's DNS management UI. The domain was also added to the Vercel project's custom-domain settings.

2. **`APP_URL` constant updated.** `app/api/cron/route.ts` line 9: `const APP_URL = "https://app.myreturnwindow.com"`. Both downstream usages (order-detail link in reminder body, admin-summary link) reference the constant and picked up the change automatically.

3. **`auth.ts` split for login sender.** Lines 23 and 27 changed from bare `process.env.REMINDER_FROM_EMAIL` to `process.env.LOGIN_FROM_EMAIL ?? process.env.REMINDER_FROM_EMAIL`. No other send sites touched — the cron route, weekly coverage route, and `lib/adminNotify.ts` all continue reading `REMINDER_FROM_EMAIL` directly.

4. **Postmark domain verification.** Four DNS records at GoDaddy: DKIM TXT, `pm-bounces` CNAME → `pm.mtasv.net`, root SPF (`v=spf1 include:spf.mtasv.net ~all`), DMARC TXT (`v=DMARC1; p=none`). Postmark's Domains tab confirmed green for `myreturnwindow.com`.

5. **Env vars in Vercel.** `REMINDER_FROM_EMAIL` updated to `reminders@myreturnwindow.com`. `LOGIN_FROM_EMAIL` added as `hello@myreturnwindow.com`. Both set for Production and Preview environments.

6. **Inbound webhook URL not changed.** Still registered against `returns-assistant.vercel.app/api/inbound`. Intentional — see "Why this design" above. Documented here so a future cleanup pass knows this was a deliberate call, not an oversight.

## How to know it works

1. Browse to `https://app.myreturnwindow.com` — dashboard loads, existing session carries through, no redirect loop or auth failure.
2. Request a magic link at the login page — the email arrives from `hello@myreturnwindow.com`, lands in the inbox (not spam), the link resolves correctly and starts a session.
3. Check a reminder email body — the order-detail link reads `https://app.myreturnwindow.com/orders/<id>`, not the old Vercel URL.
4. `REMINDER_FROM_EMAIL` runtime test: a one-off Postmark send read the env var at runtime and confirmed the production value is `reminders@myreturnwindow.com` — the email was sent to the admin inbox and arrived correctly (Postmark MessageID `cda40e27-372d-468a-a013-eb78ac7e4761`).
5. Forwarding an email to the inbound address still works normally — the webhook URL was intentionally left unchanged and both domains serve the same app.

## Copy-paste prompts for Claude Code

**Prompt 32 — rotate the Postmark inbound webhook URL (deferred)**
